...
В статье рассматриваются несколько наборов данных ЭЭГ, два из которых наиболее значимы:

- **MAT (Mental Arithmetic Task)**: участие студентов в задаче на математический счёт.

- **STEW (Simultaneous Task EEG Workload)**: параллельное выполнение нескольких заданий. Задача:

- С помощью CiSSA разлагают исходные сигналы ЭЭГ на несколько компонент, каждая из которых несёт информацию о разных частотных диапазонах и временных структурах. После разложения из полученных компонент (или их комбинаций) вычисляют числовые признаки (энергетические, энтропийные и т.д.).

- Эти признаки затем подают на вход классификатору (например, KNN, SVM или другому алгоритму машинного обучения), чтобы автоматически определить, есть ли у человека высокая когнитивная нагрузка или низкая (иногда это бинарная классификация «нагрузка/нет нагрузки», а иногда многоуровневая: «лёгкая/средняя/высокая» нагрузка»).

\textbf{В общем, был выбран CiSSA как один из методов разложения по частотам.}
...
Рассматривается таяние ледников. Цель работы -- отделить долгосрочную тенденцию от сезонных сигналов. Это позволяет корректно выделить сезонные изменения в визуальной стратиграфии, связанные с включениями пыли и морской соли.

Тут просто взяли длину окна 10, так как "увеличение длины окна выше этого значения не оказывало значительного влияния на результаты"

\textbf{В общем, был выбран CiSSA как метод разложения по частотам для выделения периодики. С таким же успехом можно было выбрать SSA}
...
\section{Сравнение алгоритмов SSA, GSSA, Фурье и CiSSA}
...
\subsubsection{Линейные фильтры}
...
\begin{table}[H]
	\caption{MSE разложения ряда $\TS = \TS_{\sin} + \TS_{\cos} + \TS_{\mathrm{noise}}$ для $\SSA$ + $\GSSA$ с $\alpha = 0.5$}
	...
	\begin{tabular}{c|ccc}
		...
		Метод & $\TS_{\sin}$ & $\TS_{\cos}$ & $\TS$ \\ 
		\hline
		$\SSA$ + $\GSSA$, $\alpha = 0.5$ & 1.06e-03 & 1.12e-03 & 7.15e-04 \\ 
		\hline
	\end{tabular}
\end{table}

Анализ данных, представленных в таблице \ref{tab:errs_ssa_gssa_united}, позволяет сделать вывод, что комбинирование алгоритмов привело к улучшению как в выделении полезного сигнала, так и в разделении компонент между собой. Полученные результаты демонстрируют более высокую точность по сравнению с данными, приведёнными в таблице \ref{tab:errs_ssa_gssa}.
