...
s <- ssa(IP_values_slice, L = 192)
e <- eossa_new(s, nested.groups = list(groups_r), clust_type = "distance")
...
plot(s, type="paired", idx = 1:30)
plot(s, type="series", groups=1:13)
...
```{r}
# Увеличиваем размер графического окна
dev.new(width = 8, height = 10)  # Ширина 8 дюймов, высота 10 дюймов

# Устанавливаем макет для двух графиков: один под другим
par(mfrow = c(2, 1), mar = c(5, 5, 4, 2) + 0.1, oma = c(0, 0, 2, 0))

# Первый график: Расширение временного ряда IP values
IP_values_slice |> extend(192) |>
  plot(type="l", lwd = 3, ylab="Значения", xlab = "Индексы", main="Расширение временного ряда IP values")
lines((192:(length(IP_values_slice) + 192 -1)) + 1, IP_values_slice, type="l", col="red")

# Второй график: Расширение временного ряда sin
n <- 96*2 + 6
x <- 0:(n-1)
y <- sin(2*pi/12 * x)
L <- 96+6
...
interesting_sin <- function(x){
  ...
  sin(x[x <= 15]*2*pi/12) |> plot(type="l")
  
  second_seq <- x[15 < x & x <= 45 ]/15 * sin(x[15 < x & x <= 45 ]*2*pi/12)
  second_seq |>plot(type = "l")
...
interesting_sin(1:50) |> plot(type="l")
```
...
r_fft_grouped_extended <- grouping_cissa(r_fft_extended,
                    groups = list(
                      # sesonal_sin = c(1/12-eps, 1/12+eps),
                      sesonal_sin = c(0-eps, 1/12+treshold),
                      # sesonal_cos = c(1/3-eps, 1/3+eps)
                      sesonal_cos = c(1/3-treshold, 1/2 + eps),
                      all = c(-10, 10)
                    )
                    )$t_series


# grouping_cissa(r_fft_extended,
#                     groups = list(
#                       # sesonal_sin = c(1/12-eps, 1/12+eps),
#                       sesonal_sin = c(0-eps, 1/12+treshold),
#                       # sesonal_cos = c(1/3-eps, 1/3+eps)
...
data <- data.frame(
  Метод = c("SSA, \nLw, Kw in N",
            ...
            "Fourier, Nw not in N",
            "CiSSA, Lw not in N",
            "CiSSA extended, Lw not in N",
            "Fourier extended, Nw not in N"),
