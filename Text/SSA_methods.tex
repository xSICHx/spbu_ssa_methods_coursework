...
\right\},
\] где $\varepsilon_n \sim \mathrm N(0, 0.1^2)$ с $A_1 = \frac{1}{200}$, $A_2 = \frac{1}{100}$, $\omega_1 = \frac{1}{12}$, $\omega_2 = \frac{1}{3}$, $N = 96 \cdot 2$. Восстановление проводилось по диапазонам $[\omega_i \pm 10 \cdot \frac{1}{193}]$. Было проведено 100 экспериментов и результаты усреднялись для соответствующих компонент и $L$. В качестве $L$ рассмотрим значения, кратные частотам компонентам сигнала. Конкретнее,  $L = 12, 24, 36, 48, 60, 72, 84, 96, 192$.
...
По рисунку \ref{fig:cissa_error_expcos_noised_depends_on_L} снова видна тенденция роста ошибки с увеличением окна. 

Таким образом, можно сделать вывод, что если временной ряд не состоит исключительно из периодик с постоянной амплитудой, то нужно брать $L$ как можно меньше.
