...
%	\setbeameroption{show notes}
	...
	\begin{frame}{Вложенный вариант SSA. EOSSA}
		...
		\end{definition}
		\bigskip
		$\EOSSA$ \parencite{golyandina2023intelligent} является вложенным вариантом $\SSA$.
	\end{frame}
...
%	\end{frame}
	...
	\begin{frame}{Пример. Точная разделимость}
		...
		$\EOSSA$ исправляет ситуацию для $\SSA$.
	...
	\begin{frame}{Пример. Асимптотическая разделимость}
		...
		При домножении на экспоненты периодик, все результаты ухудшились кроме $\EOSSA$. \textbf{Фурье} и $\CISSA$ значительно ухудшились в точности разделения.
	\end{frame}
