...
ggsave(
  filename = "cissa_errors_plot_expcos_noised.png",
  plot = p,
  width = 10,       # ширина в дюймах
  height = 6,       # высота в дюймах
  dpi = 600        # dots per inch — высокая детализация
...
good_experiment <- function(
  ...
  # Группировки (одни и те же группы применяются ко всем комбинациям данного метода):
  groups_ssa = list(),
  ...
  for (idx in seq_along(runs)) {
    ...
    # Берём первые n_val точек из y_full
    y_partial <- y_full[1:n_val]
    
    # сюда будем помещать восстановленные компоненты
    ...
    # Вычисляем суммарную ошибку по y_partial
    rec_sum  <- Reduce("+", rec_list)
    total_err <- mse_fun(y_partial, rec_sum)
