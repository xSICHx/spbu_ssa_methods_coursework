...
\begin{comment}
\label{comm:proector}
$U_k U_k^H + U_{L+2-k} U_{L+2-k}^H$ является оператором проектирования на подпространство, которое порождено синусами и косинусами с частотой $w_k = \frac{k-1}{L}$.
\end{comment}
% \begin{proof}
...
%\subsection{Асимптотическая эквивалентность методов}
...
Функциональным временным рядом будем называть ${\pmb x}\in\mathbb{H}^k$: ${\pmb x}(s)=\begin{pmatrix} x_1(s),\ldots,x_k(s)\end{pmatrix}^\top$, $x_i\in\mathbb{H} =\mathcal{L}^2([0,1])$, $s\in[0,1]$. Скалярное произведение: $\langle\pmb x,\pmb y\rangle_{\mathbb{H}^k}=\sum_{i=1}^k\langle x_i,y_i\rangle$, $\langle x,y\rangle=\int_0^1 x(s)y(s)ds$. Также определим тензорное произведение: для $x\in\mathbb{H}_1$, $y\in\mathbb{H}_2$, оператор $x\otimes y:\mathbb{H}_1\to\mathbb{H}_2$, $(x\otimes y)h=\langle x,h\rangle y$, $h\in\mathbb{H}_1$.
...
Кроме того, $\mathcal{X}=\EuScript{T}\textbf{y}_N$, где $\EuScript{T}$ — оператор из $\mathbb{H}^N$ в $\mathbb{H}_H^{L\times K}$. Вычисление $\mathcal{X} \pmb{a}$ в заданной точке $s\in [0,1]$ эквивалентно матричному произведению ${\bf X}(s)\pmb{a}$, где ${\bf X}(s)$ — это $L \times K$ ганкелева матрица, заданная как
...
Для заданного $q$ ($1\leq q\leq m$) используется $\EuScript{T}^{-1}:\mathbb{H}_H^{L\times K}\to\mathbb{H}^N$ для преобразования оператора $\mathcal{X}_{I_q}$ из \eqref{eq:grouping} в $\tilde{\bf y}_N^q$.
...
Как итог: $\tilde{\bf y}_N^q=\EuScript{T}^{-1}\tilde{\mathcal{X}}_{I_q}$.
...
$\TS = \TS_{\sin} + \TS_{\cos} +\TS_{\mathrm{noise}} = A_1 \sin(2\pi \omega_1 n ) + A_2  \cos(2\pi \omega_2 n ) + \varepsilon_n$, где $\varepsilon_n \sim \mathrm N(0, 0.1^2)$, $A_1 = 1$, $A_2 = 0.5$, $\omega_1 = \frac{1}{12}$, $\omega_2 = \frac{1}{3}$, $N = 96 \cdot 2$. Было проведено 100 экспериментов и результаты усреднялись для соответствующих компонент и $L$. В качестве $L$ рассмотрим значения, кратные частотам компонентам сигнала. Конкретнее,  $L = 12, 24, 36, 48, 60, 72, 84, 96, 192$.
...
Как видно по рисунку \ref{fig:cissa_error_cos_noised_depends_on_L}, ошибка в основном растет с увеличением $L$, но незначительно.

Также рассмотрим пример с экспоненциальной модуляцией: $\TS = \TS_{e\cdot\sin} + \TS_{e\cdot\cos} = e^{A_1 n } \sin(2\pi \omega_1 n ) + e^{A_2 n} \cos(2\pi \omega_2 n ) + \varepsilon_n$, где $\varepsilon_n \sim \mathrm N(0, 0.1^2)$ с $A_1 = 200$, $A_2 = 100$, $\omega_1 = \frac{1}{12}$, $\omega_2 = \frac{1}{3}$, $N = 96 \cdot 2$. Было проведено 100 экспериментов и результаты усреднялись для соответствующих компонент и $L$. В качестве $L$ рассмотрим значения, кратные частотам компонентам сигнала. Конкретнее,  $L = 12, 24, 36, 48, 60, 72, 84, 96, 192$.
...
По рисунку \ref{fig:cissa_error_expcos_noised_depends_on_L}  также видна тенденция к росту ошибки с увеличением длины окна.
...
\textbf{\large{Пример.}} Будем разделять временной ряд $\TS = \TS_{\sin} + \TS_{\cos} = \left\{ \sin{\frac{2\pi}{12}n} + \frac{1}{2}\cos{\frac{2\pi}{3}n} \right\}$.
...
\textbf{\large{Пример.}} Будем разделять временной ряд 
\(\TS = \TS_{e \cdot \sin} + \TS_{e \cdot \cos} = \{e^{n/200}\sin(2\pi n/12) + e^{n/100}\cos(2\pi n/3)\}.\)
...
\textbf{\large{Пример.}} Рассмотрим ряд из предыдущего примера и добавим к нему тренд. $\TS = \TS_{c} + \TS_e + \TS_{\sin} + \TS_{\cos} = 1 + e^{\frac{n}{100}} + \sin{\frac{2\pi}{12}n} + \frac{1}{2}\cos{\frac{2\pi}{3}n}$.
...
\begin{tabular}{l|c|cccc}
  ...
  \hline
  $\SSA$      & (1)  & 2.6e-04 & 3.2e-04 & 1.2e-04 & 6.1e-04 \\ 
  $\SSA$      & (2)  & 2.3e-04 & 3.2e-04 & 1.2e-04 & 6.0e-04 \\ 
  \hline
  $\EOSSA$    & (3)  & 1.9e-04 & 3.1e-04 & 1.2e-04 & 6.0e-04 \\ 
  $\EOSSA$    & (4)  & 2.0e-04 & 3.1e-04 & 1.2e-04 & 6.1e-04 \\ 
  \hline
  $\FOURIER$  & (5)  & 2.0e-01 & 5.1e-02 & 4.8e-03 & 1.5e-01 \\ 
  $\FOURIER$  & (6)  & 2.2e-01 & 6.3e-02 & 1.0e-02 & 1.5e-01 \\ 
  \hline
  $\FOURIER_{\text{ext}}$ & (7)  & 4.5e-03 & 3.1e-03 & 9.2e-04 & 8.4e-03 \\ 
  $\FOURIER_{\text{ext}}$ & (8)  & 7.0e-03 & 4.1e-03 & 1.0e-03 & 1.0e-02 \\ 
  \hline
  $\CISSA$    & (9)  & 3.8e-03 & 1.9e-03 & 9.7e-04 & 3.9e-03 \\ 
  $\CISSA$    & (10) & 1.0e-01 & 5.3e-03 & 4.9e-04 & 6.4e-02 \\ 
  $\CISSA$    & (11) & 1.1e-01 & 1.5e-02 & 1.0e-02 & 7.1e-02 \\ 
  \hline
  $\CISSA_{\text{ext}}$ & (12) & 1.2e-03 & 1.4e-03 & 9.6e-04 & 3.2e-03 \\ 
  $\CISSA_{\text{ext}}$ & (13) & 1.9e-03 & 1.2e-03 & 4.3e-04 & 2.4e-03 \\ 
  $\CISSA_{\text{ext}}$ & (14) & 1.9e-03 & 1.3e-03 & 1.3e-03 & 3.2e-03 \\ 
  \hline
\end{tabular}
\end{table}
\begin{table}[H]
...
По таблице \ref{tab:trend_exp_mod_results} видно, что алгоритмы $\CISSA$ и Фурье без модификаций достаточно плохо определяют тренд. Ситуация с выделением тренда улучшается при использовании процедуры расширения ряда, однако есть недостаток у такого решения. 
...
\subsubsection{Постоянная амплитуда}

\textbf{\large{Пример.}} Будем разделять временной ряд $\TS = \TS_{\sin} + \TS_{\cos} + \TS_{\mathrm{noise}}$ = \\$ \left\{ \sin{\frac{2\pi}{12}n} + \frac{1}{2}\cos{\frac{2\pi}{3}n}+ \varepsilon_n \right\} $, где $\varepsilon_n \sim \mathrm N(0, 0.1^2)$. Проводилось 100 экспериментов с усреднением результата.
...
\begin{tabular}{l|c|ccc}
  ...
  \textbf{Метод} & \textbf{Условия} & \(\MSE(\TS_{\sin})\) & \(\MSE(\TS_{\cos})\) & \(\MSE(\TS)\) \\ 
  \hline
  $\SSA$    & (1) & 1.9e-04 & 4.4e-04 & 6.3e-04 \\ 
  $\SSA$    & (2) & 2.0e-04 & 4.4e-04 & 6.3e-04 \\ 
  \hline
  $\EOSSA$  & (3) & 1.9e-04 & 4.4e-04 & 6.3e-04 \\ 
  $\EOSSA$  & (4) & 1.9e-04 & 4.4e-04 & 6.3e-04 \\ 
  \hline
  $\FOURIER$    & (5) & 4.2e-04 & 4.2e-04 & 8.4e-04 \\ 
  $\FOURIER$ & (6) & 2.8e-03 & 6.9e-03 & 9.6e-03 \\ 
  \hline
  $\CISSA$  & (9) & 8.5e-04 & 1.2e-03 & 2.1e-03 \\ 
  $\CISSA$ & (10) & 4.6e-04 & 4.4e-04 & 9.0e-04 \\ 
  $\CISSA$ & (11) & 4.0e-03 & 9.6e-03 & 1.2e-02 \\ 
  \hline
\end{tabular}
\end{table}
...
По таблице \ref{tab:exact_noised_results} можно сделать лишь те же результаты, что и без шума.

\subsubsection{Эксп амплитуда}


\textbf{\large{Пример.}} Будем разделять временной ряд 
$\TS = \TS_{e \cdot \sin} + \TS_{e \cdot \cos} + \TS_{\mathrm{noise}} = \{e^{n/200}\sin(2\pi n/12) + e^{n/100}\cos(2\pi n/3)+ \varepsilon_n \} $, где $\varepsilon_n \sim \mathrm N(0, 0.1^2)$. Проводилось 100 экспериментов с усреднением результата.
...
\begin{tabular}{l|c|ccc}
  ...
  \textbf{Метод} & \textbf{Условия} & \(\MSE(\TS_{\sin})\) & \(\MSE(\TS_{\cos})\) & \(\MSE(\TS)\) \\ 
  \hline
  $\SSA$    & (1) & 2.2e-04 & 4.7e-04 & 6.6e-04 \\ 
  $\SSA$    & (2) & 2.7e-04 & 5.7e-04 & 6.6e-04 \\ 
  \hline
  $\EOSSA$  & (3) & 2.0e-04 & 4.7e-04 & 6.6e-04 \\ 
  $\EOSSA$  & (4) & 2.0e-04 & 4.6e-04 & 6.6e-04 \\ 
  \hline
  $\FOURIER$    & (5) & 1.8e-02 & 1.5e-01 & 1.6e-01 \\ 
  $\FOURIER$ & (6) & 1.5e-02 & 2.2e-01 & 2.3e-01 \\ 
  \hline
  $\CISSA$  & (7) & 9.9e-04 & 2.7e-03 & 3.5e-03 \\ 
  $\CISSA$ & (8) & 6.7e-03 & 8.6e-02 & 8.7e-02 \\ 
  $\CISSA$ & (9) & 1.4e-02 & 2.8e-01 & 2.6e-01 \\ 
  \hline
\end{tabular}
\end{table}
...
С добавлением шума результаты, ожидаемо, ухудшились. Однако можно заметить, что $\SSA$ наилучшим образом разделяет как сигнал от шума, так и компоненты между собой. Остальные методы имеют точность сильно меньше
...
Заметно, что с трендом хуже всего справляется $\FOURIER$. А лучше всех себя показывает $\SSA$ и $\EOSSA$ во всех смыслах.
...
Работа посвящена изучению трех существующих подходов, которые предлагаются для построений модификаций метода анализ сингулярного спектра (SSA) для анализа временных рядов. Целью работы является анализ рассмотренных подходов с точки зрения теории SSA, выявление их сильных и слабых сторон, а также использование их сильных сторон в новых модификациях. Были рассмотрены следующие модификации. Первая – Generalized SSA (GSSA), которая введением весов улучшает разделимость в отсутствии шума. Это свойство было объяснено с помощью рассмотрения применения SSA как системы линейных адаптивных фильтров. Была предложена новая модификация, объединяющая аппроксимационные свойства SSA и преимущества GSSA. Вторая модификация – это Circular SSA (CiSSA), где адаптивный базис, который строится в SSA, заменен на фиксированный базис Фурье в траекторном пространстве ряда. Естественные проблемы в CiSSA связаны с выделением тренда и амплитудно-модулированных гармоник, однако авторы методы делали акцент на простоте автоматического выделения компонент с заданными частотными диапазонами. В работе показано, что современные методы автоматической идентификации компонент в базовом варианте SSA справляются с выделением компонент, как минимум, не хуже, причем точность лучше. И третья модификация – Functional SSA (FSSA), которая оказалась, по сути, методом для анализ двумерных данных. При сравнении с 2D-SSA в работе показано, методы сравнимы по точности, а существенное преимущество FSSA, демонстрируемое его авторами, связано с тем, что авторы его сравнивали с SSA для многомерных рядов, игнорируя регулярное поведение по аргументу, рассматриваемому авторами как непрерывный параметр. Работа дополнена большим количеством численных экспериментов для демонстрации результатов.
